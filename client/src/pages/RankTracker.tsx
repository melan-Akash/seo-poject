...
export default function RankTracker() {
    ...
    const fetchKeywords = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.get("/api/rank/keywords");
            ...
            toast.error(error.response?.data?.message || "Failed to load keywords");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAdd = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!newKeyword.trim() || !newUrl.trim()) {
            setAddError("Keyword and URL are required");
            return;
        }
        ...
        try {
            const response = await api.post("/api/rank/keywords", {
                keyword: newKeyword,
                url: newUrl
            });
            
            if (response.data.success) {
                toast.success("Keyword tracking started successfully!");
                setShowAddModal(false);
                setNewKeyword("");
                setNewUrl("");
                await fetchKeywords();
            } else {
                setAddError(response.data.message || "Failed to add keyword");
                toast.error(response.data.message || "Failed to add keyword");
            }
        } catch (error: any) {
            console.error("Error adding keyword:", error);
            const errorMessage = error.response?.data?.message || "Failed to add keyword. Please try again.";
    ...
    const handleRefresh = async (id: string) => {
        ...
        try {
            ...
            if (response.data.success) {
                toast.success("Ranking refresh initiated!");
                await fetchKeywords();
            } else {
    ...
    const handleDelete = async (id: string) => {
        ...
        setDeleting(id);
        try {
            const response = await api.delete(`/api/rank/keywords/${id}`);
            if (response.data.success) {
                toast.success("Keyword tracking deleted successfully");
                await fetchKeywords();
            } else {
    ...
    const handleToggle = async (id: string) => {
        try {
            const response = await api.put(`/api/rank/keywords/${id}/toggle`);
            if (response.data.success) {
